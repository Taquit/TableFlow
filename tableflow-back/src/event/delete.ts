...
export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> => {
    ...
    try {
        ...
        const result = await client.query('DELETE FROM "Event" WHERE id = $1 RETURNING id;', [parseInt(id)]);
        ...
        return { statusCode: 200, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: "Evento eliminado", id: result.rows[0].id }) };
    } catch (error) {
